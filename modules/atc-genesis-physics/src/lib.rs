...
pub use water::{WaterBody, WaterConfig, WaterError, WaterSample, WaterSurface};
...
impl PhysicsSimulation {
 ...
 pub fn bodies(&self)->&[RigidBody]{&self.bodies} pub fn body(&self,entity:EntityId)->Option<&RigidBody>{self.bodies.iter().find(|b|b.entity==entity)} pub fn body_mut(&mut self,entity:EntityId)->Option<&mut RigidBody>{self.bodies.iter_mut().find(|b|b.entity==entity)}
 ...
 pub fn add_collider(&mut self,collider:Collider){self.collisions.add(collider)} pub fn remove_collider(&mut self,entity:EntityId)->bool{self.collisions.remove(entity)} pub fn query(&self,bounds:Aabb)->Vec<EntityId>{self.collisions.query_aabb(bounds)} pub fn interpolation_alpha(&self)->f32{(self.accumulator/self.config.fixed_dt).clamp(0.0,1.0)}
}
#[derive(Default)] pub struct WorldPhysicsBridge{chunk_entities:std::collections::HashMap<WorldChunkId,EntityId>}
