//! Deterministic production stations, bounded storage, job capacity, and tool durability.
...
impl ProductionStation {
    pub fn new(id: &'static str, kind: ProductionStationKind, capacity: u16) -> Option<Self> {
        ...
        Some(Self { id, kind, capacity, active_jobs: 0, enabled: true, input: Inventory::default(), output: Inventory::default() })
    }
...
pub const WORKBENCH: ProductionStation = ProductionStation { id: "workbench", kind: ProductionStationKind::Workbench, capacity: 1, active_jobs: 0, enabled: true, input: Inventory { stacks: Vec::new() }, output: Inventory { stacks: Vec::new() } };
pub const FORGE: ProductionStation = ProductionStation { id: "forge", kind: ProductionStationKind::Forge, capacity: 1, active_jobs: 0, enabled: true, input: Inventory { stacks: Vec::new() }, output: Inventory { stacks: Vec::new() } };
pub const KITCHEN: ProductionStation = ProductionStation { id: "kitchen", kind: ProductionStationKind::Kitchen, capacity: 1, active_jobs: 0, enabled: true, input: Inventory { stacks: Vec::new() }, output: Inventory { stacks: Vec::new() } };
