...
pub fn start_production(
    ...
    match (tool_requirement, tool.as_deref()) {
        ...
        (Some(_), Some(_)) => return Err(ProductionFailure::MissingTool),
        (Some(_), None) => return Err(ProductionFailure::MissingTool),
    ...
    if let Some(t) = tool.as_deref_mut() { if !t.use_once() { return Err(ProductionFailure::ToolBroken); } }
...
mod tests {
    ...
    #[test] fn start_is_atomic_for_validation_failures() { let mut i=Inventory::default(); i.add("wood",2,10); let s=ProductionStation::new("workbench",ProductionStationKind::Workbench,1).unwrap(); assert_eq!(start_production(&RECIPE,&mut i,&s,&[],None,None),Err(ProductionFailure::MissingTechnology)); assert_eq!(i.count("wood"),2); }
    #[test] fn production_consumes_and_finishes() { let mut i=Inventory::default(); i.add("wood",2,10); let s=ProductionStation::new("workbench",ProductionStationKind::Workbench,1).unwrap(); let mut j=start_production(&RECIPE,&mut i,&s,&["construction"],None,None).unwrap(); assert_eq!(i.count("wood"),0); j.job.tick(2); let mut s=s; assert!(finish_production(&j,&mut s).is_ok()); assert_eq!(s.output.count("plank"),1); }
    #[test] fn required_tool_is_enforced() { let mut i=Inventory::default(); i.add("wood",2,10); let s=ProductionStation::new("workbench",ProductionStationKind::Workbench,1).unwrap(); let mut t=ToolState::new("hammer",2).unwrap(); let req=ProductionToolRequirement{tool_id:"hammer"}; assert!(start_production(&RECIPE,&mut i,&s,&["construction"],Some(req),Some(&mut t)).is_ok()); assert_eq!(t.durability,1); }
    #[test] fn wrong_tool_does_not_mutate_inventory() { let mut i=Inventory::default(); i.add("wood",2,10); let s=ProductionStation::new("workbench",ProductionStationKind::Workbench,1).unwrap(); let mut t=ToolState::new("axe",2).unwrap(); let req=ProductionToolRequirement{tool_id:"hammer"}; assert_eq!(start_production(&RECIPE,&mut i,&s,&["construction"],Some(req),Some(&mut t)),Err(ProductionFailure::MissingTool)); assert_eq!(i.count("wood"),2); assert_eq!(t.durability,2); }
}
