//! Deterministic reusable gameplay foundations for genre-scale systems.
...
impl CharacterState { pub fn jump(&mut self)->bool{if self.grounded&&self.stamina>=10{self.grounded=false;self.stamina-=10;true}else{false}} pub fn land(&mut self){self.grounded=true;} pub fn recover(&mut self,amount:u16){self.stamina=self.stamina.saturating_add(amount).min(self.max_stamina);} }
...
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct ReplicatedTransform { pub entity:u64,pub tick:u64,pub position:[f32;3] }
#[derive(Default, Debug)] pub struct ReplicationBuffer { pub frames:Vec<ReplicatedTransform> }
impl ReplicationBuffer { pub fn push(&mut self,frame:ReplicatedTransform){self.frames.push(frame);self.frames.sort_by_key(|f|(f.tick,f.entity));} pub fn latest(&self,entity:u64)->Option<ReplicatedTransform>{self.frames.iter().rev().find(|f|f.entity==entity).copied()} }

#[derive(Clone, Copy, Debug, PartialEq)]
pub struct RaceState { pub lap:u32,pub checkpoint:u32,pub progress:f32 }
impl RaceState { pub fn advance(&mut self,checkpoint:u32,progress:f32){if checkpoint>=self.checkpoint{self.checkpoint=checkpoint;self.progress=progress.clamp(0.0,1.0)}} pub fn finish_lap(&mut self){self.lap=self.lap.saturating_add(1);self.checkpoint=0;self.progress=0.0;} }
...
pub struct ModDescriptor { pub id:String,pub version:String,pub dependencies:Vec<String>,pub permissions:Vec<String> }
#[derive(Default)] pub struct ModRegistry { pub mods:Vec<ModDescriptor> }
impl ModRegistry { pub fn register(&mut self,m:ModDescriptor)->bool{if m.id.is_empty()||self.mods.iter().any(|x|x.id==m.id){return false} self.mods.push(m);self.mods.sort_by(|a,b|a.id.cmp(&b.id));true} pub fn contains(&self,id:&str)->bool{self.mods.iter().any(|m|m.id==id)} pub fn dependencies_present(&self,id:&str)->bool{self.mods.iter().find(|m|m.id==id).map(|m|m.dependencies.iter().all(|d|self.contains(d))).unwrap_or(false)} }

#[cfg(test)] mod tests{use super::*;
#[test]fn character_jump_is_gated(){let mut c=CharacterState::default();assert!(c.jump());assert!(!c.jump());c.land();c.recover(10);assert_eq!(c.stamina,100);}
...
#[test]fn replication_is_stable(){let mut b=ReplicationBuffer::default();b.push(ReplicatedTransform{entity:2,tick:2,position:[2.0,0.0,0.0]});b.push(ReplicatedTransform{entity:1,tick:1,position:[1.0,0.0,0.0]});assert_eq!(b.latest(2).unwrap().tick,2);}
...
#[test]fn mods_require_dependencies(){let mut r=ModRegistry::default();assert!(r.register(ModDescriptor{id:"base".into(),version:"1".into(),dependencies:vec![],permissions:vec![]}));assert!(r.register(ModDescriptor{id:"addon".into(),version:"1".into(),dependencies:vec!["base".into()],permissions:vec!["gameplay.read".into()]}));assert!(r.dependencies_present("addon"));}
}