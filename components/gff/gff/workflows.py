...
The module provides a deterministic orchestration layer. It does not call an AI
provider itself; executors are injected by the caller. This keeps provider,
credentials, and side effects outside the core workflow model.
"""
...
class WorkflowContext:
    """Data exchanged between workflow stages."""
...
class WorkflowRegistry:
    """Registry for the canonical Franchise Factory workflow catalogue."""

    def __init__(self, definitions: tuple[WorkflowDefinition, ...] | None = None) -> None:
        self._definitions = {item.id: item for item in definitions or DEFAULT_WORKFLOWS}
        self.validate()
    ...
    def validate(self) -> None:
        if not self._definitions:
            raise WorkflowError("Workflow registry must not be empty")
        for definition in self._definitions.values():
            if not definition.stages or definition.stages[0] is not WorkflowStage.INPUT:
                raise WorkflowError(f"Workflow {definition.id} must start with INPUT")
            if WorkflowStage.QUALITY not in definition.stages:
                raise WorkflowError(f"Workflow {definition.id} requires a QUALITY gate")
            if len(set(definition.stages)) != len(definition.stages):
                raise WorkflowError(f"Workflow {definition.id} contains duplicate stages")


class WorkflowEngine:
    """Execute a registered workflow using an injected stage executor."""
    ...
    def run(
        ...
        context = WorkflowContext(workflow_id=definition.id, payload=dict(payload))
        for stage in definition.stages:
            context.stage = stage
            context = executor(stage, context)
            if not isinstance(context, WorkflowContext):
                raise WorkflowError(f"Executor returned invalid context at stage {stage}")
        context.record_evidence("workflow.completed", definition.id)
        return context
...
GAME_SUBSYSTEMS: Mapping[str, tuple[str, ...]] = {
    "concept": ("genre", "target", "platforms", "usp", "core-loop", "modes", "monetization", "technical-requirements"),
    "world": ("continents", "regions", "biomes", "cities", "dungeons", "buildings", "climate", "day-night", "weather", "portals"),
    "lore": ("origin", "peoples", "factions", "wars", "timeline", "secrets", "canon", "artifacts"),
    "character": ("player", "npc", "classes", "attributes", "skills", "progression", "relationships"),
    "creature": ("anatomy", "abilities", "weaknesses", "attacks", "movement", "loot", "variants", "boss-mechanics"),
    "combat": ("melee", "ranged", "magic", "combos", "dodge", "parry", "status", "boss-phases", "pvp-balance"),
    "quest": ("main", "side", "faction", "events", "puzzles", "boss", "hidden", "dynamic"),
    "level": ("terrain", "rooms", "paths", "encounters", "loot", "checkpoints", "puzzles", "secrets", "scaling"),
    "item": ("weapons", "armor", "accessories", "consumables", "resources", "relics", "artifacts", "skins", "crafting"),
    "weapon": ("concept", "design", "3d", "animation", "vfx", "sound", "gameplay"),
    "animation": ("idle", "walk", "run", "jump", "attack", "combo", "hit", "death", "emotes", "interactions"),
    "ai-npc": ("identity", "memory", "personality", "goals", "knowledge", "behavior", "routine", "relationships"),
    "audio": ("sfx", "voice", "ambient", "music", "dynamic-music", "spatial-audio"),
    "vfx": ("fire", "water", "explosions", "magic", "energy", "portals", "weather", "abilities", "boss-effects"),
    "economy": ("loot", "resources", "crafting", "pricing", "demand", "inflation", "progression", "rewards"),
    "multiplayer": ("matchmaking", "lobby", "party", "guilds", "pvp", "pve", "raids", "leaderboards", "seasons", "server"),
    "testing": ("bugs", "exploits", "levels", "combat", "quests", "economy", "performance", "network", "ui", "progression", "ai-playtests"),
    "liveops": ("telemetry", "retention", "funnel", "balance", "bugs", "seasons", "events", "content"),
}
