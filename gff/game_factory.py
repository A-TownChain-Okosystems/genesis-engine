...
class GameFactoryNode:
    """One production domain and the artifact kinds it consumes/produces."""
...
class GameFactoryGraphError(ValueError):
    """Raised when the Game Factory dependency graph is invalid."""
    ...
    for node in graph:
        for requirement in node.requires:
            if requirement not in produced:
                raise GameFactoryGraphError(
                    f"Node {node.id} requires {requirement}, but no producer exists"
                )


def topological_order(graph: tuple[GameFactoryNode, ...] = GAME_FACTORY_GRAPH) -> tuple[str, ...]:
    """Return a deterministic order or fail closed on a dependency cycle."""
...
def dependency_refs(
    node: GameFactoryNode,
    artifacts: tuple[ArtifactRef, ...],
) -> tuple[ArtifactRef, ...]:
    """Resolve all required inputs from produced artifact references."""
