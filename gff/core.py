...
class FranchiseStatus(Enum):
    """FranchiseStatus aus gff_core_ad20.atc."""
...
class PipelineStatus(Enum):
    """PipelineStatus aus gff_core_ad20.atc."""
...
class FranchiseBlueprint:
    """FranchiseBlueprint aus gff_core_ad20.atc."""
...
@dataclass
class Franchise:
    """Franchise-Record aus gff_core_ad20.atc."""
...
@dataclass
class PipelineStage:
    """PipelineStage aus gff_core_ad20.atc (Default-Pipeline: 10 Stufen)."""
...
@dataclass
class StageResult:
    """Ergebnis eines Executor-Aufrufs (ehrfuerchtig: success + note, keine Fake-Daten)."""

    success: bool
    note: str = "dry-run"


# Executor-Protokoll: Callable[(stage, franchise, franchise_context) -> StageResult]
...
def default_pipeline() -> list[PipelineStage]:
    """Default-Pipeline aus gff_core_ad20.atc init_gff() (10 Stufen, Reihenfolge verbindlich)."""
    return [
...
class GFFCore:
    """GFFCore aus gff_core_ad20.atc: Registry + Pipeline + Events."""
    ...
    def __init__(self, spec_dir=None, version: str = "1.0.0"):
        ...
        self.factory_specs: dict[int, FactorySpec] = (
            load_specs(spec_dir) if spec_dir is not None else {}
        )
    ...
    def create_franchise(self, blueprint: FranchiseBlueprint, *, now: float | None = None) -> str:
        """create_franchise aus AD-20: fid = sha256(name, timestamp)."""
        ...
        self.franchises[fid] = Franchise(
            id=fid,
            name=blueprint.name,
            universe=f"{blueprint.name} Universe",
    ...
    def run_pipeline(self, franchise_id: str, *, executor: Executor = _noop_executor) -> bool:
        """run_pipeline aus AD-20: Stufen in order, disabled -> Skipped, Progress-Update."""
    ...
    def reset_pipeline(self) -> None:
        """Reset fuer naechste Franchise (Stufen zurueck auf Pending)."""
